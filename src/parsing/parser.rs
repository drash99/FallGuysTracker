...
impl Parser {
    pub fn new(file: File) -> Parser {
        Parser {
            ...
            file,
            parsed: Vec::new(),
            regex_spawn: Regex::new(r"^[0-9/_.:-]*: \[\w*\] Finalising spawn for player FallGuy \[(\d*)\]").unwrap(),
            regex_spawn_bot: Regex::new(r"^[0-9/_.:-]*: \[\w*\] Finalising spawn for player PB_FallGuyBot \[(\d*)\]+").unwrap(),
            regex_spawn2: Regex::new(r"^[0-9/_.:-]*: \[\w*\] Adding Spectator target \w*[. ]*([^(]*)\((\w*)\) with Party ID: ([\d ]*) Squad ID: (\d*) and playerID: (\d*)+").unwrap(),
